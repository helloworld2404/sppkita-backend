        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <a href="#" data-bs-toggle="modal" data-bs-target="#tambah" class="btn btn-primary btn-sm">Tambah Siswa</a>
                </div>
                ...
                <div class="modal-body">
                    <form action="{{ route('siswa.update', $s->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Kelas</label>
                            <select class="form-select @error('kelas_id') is-invalid @enderror" name='kelas_id' aria-label="Pilih Kelas">
                                <option value="{{ $s->kelas_id }}" selected>
                                    {{ $s->nama_kelas }} - {{ $s->kompetensi_keahlian }}
                                </option>
                                @foreach($kelas as $k)
                                    @if($k->id != $s->kelas_id) <!-- Exclude the currently selected class -->
                                        <option value="{{ $k->id }}">
                                            {{ $k->nama_kelas }} - {{ $k->kompetensi_keahlian }}
                                        </option>
                                    @endif
